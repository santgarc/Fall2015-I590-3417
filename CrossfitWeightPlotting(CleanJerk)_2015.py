...
# Arrays to store Crossfit Regional cleanandjerk weight
regional_cleanandjerk_weight = []
regional_competitor_age = []

# Arrays to store Crossfit Regional cleanandjerk weight conversion 
# from string to decimal/integers

clean_regional_competitor_age_int = []
clean_regional_cleanandjerk_weight = []

# Arrays to store Crossfit GAmes 2015 cleanandjerk weight
games2015_cleanandjerk_weight = []
games2015_competitor_age = []

# Arrays to store Crossfit GAmes 2015 cleanandjerk weight conversion 
# from string to decimal/integers
clean_games2015_cleanandjerk_weight = []
...
f = open('./crossfitRegoinals_2015(Clean_CleanJerk).csv')
csv_f = csv.reader(f)

for row in csv_f:
    regional_cleanandjerk_weight.append(row[18])
    regional_competitor_age.append(row[30])
#    regional_cleanandjerk_weight_decimal.append(row[2])
...
for row in csv_f:
    games2015_cleanandjerk_weight.append(row[18])
    games2015_competitor_age.append(row[30])

            
#Stores data from Crossfit Regional Spreadsheet
for row in regional_cleanandjerk_weight:
    if(row != 'Clean & Jerk'):
        clean_regional_cleanandjerk_weight.append(int(row))
        
for row in regional_competitor_age:
    if(row != 'Age'):
      clean_regional_competitor_age_int.append(int(row))  
                    
for row in games2015_cleanandjerk_weight:
    if(row != 'Clean & Jerk'):
        clean_games2015_cleanandjerk_weight.append(int(row))
...
#for i in range(0, len(clean_regional_cleanandjerk_weight)):
# print (dt.datetime.strptime(clean_regional_cleanandjerk_weight[i], '%Y-%m-%d %H:%M:%S'))

#Concantenates all information into a 2D array for plotting
for i in  itertools.izip(clean_regional_cleanandjerk_weight, clean_regional_competitor_age_int):
    i = (i)
    all_regional_weight.append(i)
    
for i in  itertools.izip(clean_games2015_cleanandjerk_weight, clean_games2015_competitor_age):
    i = (i)
    all_games2015_weight.append(i)


print (clean_regional_cleanandjerk_weight)
...
#xy_chart.add('Games 2015', all_games_cleanandjerk_weight)

xy_chart.render_to_file("CrossfitMaleCleanandJerk_2015.svg")
